...
/**
 * API endpoint to store customer information and forward to n8n webhook
 */
export async function POST(request: Request) {
  try {
    ...
    // Standard field names used across all requests:
    // - customerName (was previously 'name' in some forms)
    // - customerEmail (was previously 'email')
    // - customerPhone (was previously 'phone')
    // - customerCompany (was previously 'company')
    // - requestType ('agent', 'service', 'product', or 'contact')
    const transformedPayload = transformFormData(payload);
    
    // Log the transformed payload for debugging
    console.log('Transformed payload to send:', JSON.stringify(transformedPayload, null, 2));
    ...
    console.error('Error in store-customer-info API route:', error);
    return NextResponse.json(
