 ...
 * MainView Component
 * 
 * This component renders the WebAgent in the main content area of the page
 */
const MainView: React.FC<MainViewProps> = ({ isHomePage = false }) => {
  const { status, actions } = useWebAgent();
  // Track if there's an active conversation
  const [hasActiveConversation, setHasActiveConversation] = useState(false);
  // Track if this is the first load to only show loading indicator initially
  const [isFirstLoad, setIsFirstLoad] = useState(true);
  // Track if voice is muted
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'agent_voice_muted') {
        ...
        console.log('MainView: Syncing mute state from storage event:', newMuteState);
    ...
    const handleMuteEvent = (e: CustomEvent) => {
      if (e.detail && typeof e.detail.muted === 'boolean') {
        console.log('MainView: Syncing mute state from custom event:', e.detail.muted);
    ...
    const intervalCheck = setInterval(() => {
      ...
      if (storedState !== isVoiceMuted) {
        console.log('MainView: Detected mute state change in localStorage:', storedState);
  ...
  // Update talking animation state when speaking status changes
  useEffect(() => {
    if (status.isSpeaking) {
      // Dispatch events when speaking starts
      window.dispatchEvent(new CustomEvent('anaya-speaking-start'));
      window.postMessage({ type: 'anaya-speaking', speaking: true }, '*');
    } else {
      // Dispatch events when speaking stops
  ...
  useEffect(() => {
    ...
    if (status.isInitialized && !status.isLoading) {
      ...
      if (storedMuteState !== isVoiceMuted) {
        console.log('MainView: Syncing initial mute state from localStorage:', storedMuteState);
  ...
  const handleRequestMicrophonePermission = useCallback(() => {
    console.log('MainView: Starting conversation, which will trigger permission request');
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Escape' && status.isActive) {
      // Toggle mute instead of pausing when Escape is pressed
      toggleVoiceMute();
    }
  }, [status.isActive]);
  ...
  const toggleVoiceMute = useCallback(() => {
    console.log("MainView: Toggling voice mute state from", isVoiceMuted, "to", !isVoiceMuted);
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        console.log("MainView: Updated localStorage mute state:", newMuteState);
      ...
      try {
        ...
        console.log("MainView: Called toggleVoiceMute with value:", newMuteState);
      } catch (toggleErr) {
  ...
  return (
    <div onKeyDown={handleKeyDown} className="w-full max-w-md mx-auto flex items-center justify-center bg-transparent">
      {/* Main button container only */}
      <div className="flex flex-col items-center justify-center w-full">
        {/* Main button container with ripple effect container */}
        <div className="relative" style={{ overflow: 'visible' }}>
          {/* RippleTalk effect - water ripple effect when speaking */}
          {status.isSpeaking && !isVoiceMuted && <RippleEffect scale={1.2} />}

          {/* Main button with glow */}
          <div className="w-16 h-16 flex items-center justify-center rounded-full bg-blue-600 shadow-lg transition-all duration-300 relative z-10">
            {/* Subtle glow effect */}
            <div className="absolute inset-0 -z-5 rounded-full bg-blue-500/20 blur-md"></div>
            
            {/* Voice status indicator - small badge on button */}
            {isVoiceMuted && status.isActive && (
              <div className="absolute -top-1 -right-1 z-20 bg-red-500 rounded-full w-5 h-5 flex items-center justify-center border border-slate-800">
            ...
            {/* Button SVG */}
            <button
              onClick={status.isActive ? toggleVoiceMute : 
                (status.error && status.error.includes('Voice features disabled') ? 
                 () => setIsPermissionModalOpen(true) : handleStartConversation)}
              disabled={showLoadingSpinner || !status.isInitialized}
              className="w-full h-full rounded-full flex items-center justify-center focus:outline-none disabled:opacity-50 bg-blue-600 hover:bg-blue-500 transition-all duration-300"
              aria-label={status.isActive ? (isVoiceMuted ? "Unmute voice" : "Mute voice") : "Start conversation"}
            >
              {showLoadingSpinner ? (
        ...
        {status.error && 
          ...
          <div className={`mt-4 px-4 py-2 rounded-full backdrop-blur-sm text-sm shadow-lg ${
            status.error && status.error.includes('Voice features disabled') 
              ? 'bg-blue-900/40 text-blue-300 border border-blue-500/30 cursor-pointer hover:bg-blue-900/60' 
              : 'bg-gray-900/70 text-red-400 border border-red-500/30'
          }`} 
            onClick={() => {
            ...
            {status.error && status.error.includes('Voice features disabled') 
              ? <span className="flex items-center"><svg className="w-4 h-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>{status.error}</span>
              : status.error}
          </div>
        )}
        
        {/* Voice status indicator - simplified */}
        <div className="mt-3 flex items-center justify-center px-3 py-1.5 rounded-full bg-slate-800/60 backdrop-blur-sm">
          <div className="flex items-center space-x-2">
            {status.isSpeaking && !isVoiceMuted ? (
              ...
              <div className="flex items-center">
                <span className="text-blue-400 font-medium">Anaya is listening</span>
                <ListeningIndicator />
              </div>
            )}
          </div>
